...
OS_SECTORS = {0, 0x800, 0x18a800, 0x1ae000, 0x1d8800, 0x1d9000, 0x735800}
...
def manifests(root=None):
    ...
    result = {}
    for name, expected in [('v2', V2), ('v3', V3)]:
...
def plan_for(prefix, mode='install', patch_set=None):
    require(mode in ('install', 'restore', 'inspect'), 'Invalid operation')
    ...
    require(observed in (STOCK, V2, V3),
            'Unsupported firmware. Requires exact iPod Video 5G Apple 1.3, reconnect-v2 or reconnect-v3.')
    ...
    wanted = original if mode == 'restore' else transform(original, patch_set[V3])
    ...
    require(len(plan) <= 8, 'Too many firmware sectors')
