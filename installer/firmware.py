...
def manifests(root=None):
    ...
    for name, expected in [('v2', V2), ('v3', V3), ('responsive', RESPONSIVE), ('smooth', SMOOTH),
                           ('v4', V4), ('smooth_v2', SMOOTH_V2), ('fast', FAST), ('efficient', EFFICIENT), ('efficient_v2', EFFICIENT_V2)]:
...
def plan_for(prefix, mode='install', patch_set=None):
    require(mode in ('install', 'responsive', 'smooth', 'fast', 'efficient', 'restore', 'inspect'), 'Invalid operation')
    ...
    require(observed in (STOCK, V2, V3, RESPONSIVE, SMOOTH, V4, SMOOTH_V2, FAST, EFFICIENT, EFFICIENT_V2),
    ...
    target = {'responsive': RESPONSIVE, 'smooth': SMOOTH_V2, 'fast': FAST, 'efficient': EFFICIENT_V2}.get(mode, V4)
