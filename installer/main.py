...
def main():
    ...
    p.add_argument('mode', nargs='?', choices=['install', 'responsive', 'smooth', 'fast', 'efficient', 'drawing_previous', 'restore', 'inspect'], default='install')
    ...
    try:
        ...
        if args.mode == 'fast':
            ...
            print('Run this release\'s standard installer to restore original menu timing.')
        if args.mode == 'drawing_previous':
            print('Restoring drawing preview 2, retaining its pixel-copy optimization and 17 ms cadence.')
            print('The new visible-screen capture shortcut will be removed.')
        if args.mode == 'efficient':
            print('Drawing preview: reuse the visible outgoing menu, with optimized pixel copies and 17 ms updates.')
