/* Apple 5G 1.3 edge-based charging-display screen-on edge tolerance prototype.
...
u32 response_step(struct observer *s,u32 raw,u32 t,u32 valid,u32 load) {
    ...
    /* Backlight turn-on adds load; do not discard a simultaneous large
     * upward edge solely because that bit changed. Other load transitions
     * still rebase, and no smaller reconnect pair crosses a load change.
     * Freshness, startup, threshold, and confirmation remain mandatory. */
    if (changed && (changed!=8 || !(load&8))) { s->base=raw;s->pending=0;return 0; }
    if (changed) s->pending=0;
