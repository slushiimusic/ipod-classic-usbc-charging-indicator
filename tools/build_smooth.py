"""Build an optional, duration-preserving menu cadence prototype; no device I/O."""
import argparse
import hashlib
import json
...
def build(source, output, clang):
    ...
    subprocess.run([clang, '--target=armv4t-none-eabi', '-marm', '-Oz', '-ffreestanding',
                    ...
                    str(ROOT / 'src/smooth_menus.c'), '-o', str(obj)], check=True)
    with obj.open('rb') as file:
        ...
        assert elf.get_section_by_name('.symtab').get_symbol_by_name('smooth_menus')[0]['st_value'] == 0
    ...
    (output / 'osos-smooth.bin').write_bytes(after)
    manifest = dict(status='LOCAL PROTOTYPE; NOT INSTALLED; PHYSICAL SMOOTHNESS AND BATTERY IMPACT UNMEASURED',
                    ...
                    payload_bytes=len(code), original_interval_ms=30, requested_interval_ms=20,
...
if __name__ == '__main__':
    ...
    p.add_argument('--output-dir', type=Path, default=ROOT / 'build/smooth')
