"""Replay the single-paint preview against a private saved UI image.
...
CANDIDATE='d95c3ded58467b7778575d33d8e3f2065b257534ca6909134e8c08d72c932dd6'
...
def check(base,previous,candidate,snapshot,report):
    ...
    replay.SNAP=prefix[0xd64000:0x2d64000]
    old=previous.read_bytes();new=candidate.read_bytes()
    original=base.read_bytes()
    assert hashlib.sha256(original).hexdigest()=='9122a8bd1c99e2be03c29c52425849a86277ddbb778a817b64fed5cb82985ba8'
    ...
    assert len(new)==0x736000 and new[-4:]==bytes.fromhex('55aa55aa')
    assert struct.unpack_from('<I',new,0xe8)[0]==len(new)
    ...
    m=Machine(new);entry=0x735f78;guards=0
    ...
    for interval,mode,direction,gpio in itertools.product((15,17,20,30,40),range(3),range(4),range(8)):
        ...
        m.call(0x735f10,OWNER+0xbc,interval)
    ...
    for offset,value,size in ((0,0x66b298,4),(0x1c,0xfeec,2),(0x20,0x1000,4),(0xb8,0,4),(0xb8,150,4),(0xb8,600,4)):
        ...
        m.u.mem_write(OWNER+offset,value.to_bytes(size,'little'));m.call(0x735f10,OWNER+0xbc,30)
