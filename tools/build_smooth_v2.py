"""Append the deadline-based menu helper to exact reconnect-v4, offline only."""
...
p.add_argument('--output-dir',type=Path,default=ROOT/'build/smooth-v2')
...
subprocess.run([args.clang,'--target=armv4t-none-eabi','-marm','-Oz','-ffreestanding',
 '-fno-builtin','-fomit-frame-pointer','-Wall','-Wextra','-Werror','-c',str(ROOT/'src/smooth_menus_v2.c'),'-o',str(obj)],check=True)
with obj.open('rb') as f:
 e=ELFFile(f);code=bytearray(e.get_section_by_name('.text').data());sym=e.get_section_by_name('.symtab')
 offsets={s.name:s['st_value'] for s in sym.iter_symbols() if s.name in ('eligible','smooth_prepare','smooth_rearm')}
...
hooks=[(0x2028c8,0x151c24,'smooth_prepare')]+[(p,0x151694,'smooth_rearm') for p in (0x2028dc,0x202ebc,0x20300c)]
for p,old,name in hooks:
 assert struct.unpack_from('<I',before,p)[0]==branch(p,old)
 struct.pack_into('<I',after,p,branch(p,start+offsets[name]))
...
(OUT/'osos-smooth-v2.bin').write_bytes(after)
m=dict(status='OFFLINE CANDIDATE; NOT INSTALLED',input_sha256=hashlib.sha256(before).hexdigest(),
 ...
 symbols={n:hex(start+v) for n,v in offsets.items()},hooks=[hex(p) for p,_,_ in hooks],
 requested_deadline_grid_hz=60,duration_ms=300,cpu_policy_changed=False,
